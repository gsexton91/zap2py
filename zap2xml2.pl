...
%options = ();
getopts( "?aA:bB:c:C:d:DeE:Fgi:IjJ:l:Lm:Mn:N:o:Op:P:qRr:s:S:t:Tu:UwWxY:zZ:89",
    \%options );
...
if ( open( CONF, $confFile ) ) {
    ...
    while (<CONF>) {
        ...
        elsif (/^\s*lineuplocation\s*=\s*(.+)/i) {
            $lineuplocation = &rtrim($1);
        }
...
&pout(  "Downloaded "
      . &pl( $tb,   "byte" ) . " in "
      . &pl( $treq, "http request" )
      . " using "
      . &pl( $tsockt > 0 ? $tsockt : $treq, "socket" )
      . ".\n" )
  if $tb > 0;
...
my $s3 = time();
&pout( "Completed in " . ( $s3 - $s1 ) . "s (Parse: " . ( $s2 - $s1 ) . "s) " .
      keys(%stations) . " stations, " .
      keys(%programs) . " programs, $ts scheduled.\n" );
...
sub appendAsterisk {
    ...
    if ( defined( $options{A} ) ) {
        if (
            ( $options{A} =~ "new" && defined( $schedule{$station}{$s}{new} ) )
            || ( $options{A} =~ "live"
                && defined( $schedule{$station}{$s}{live} ) )
          )
        {
            $title .= " *";
        }
...
sub sortChan {
    if ( defined( $stations{$a}{order} ) && defined( $stations{$b}{order} ) ) {
        ...
        if ( $c == 0 ) {
            return $stations{$a}{stnNum} <=> $stations{$b}{stnNum};
        }
        else { return $c }
    }
    else {
...
sub enc {
    ...
    if ( !defined( $options{E} ) || $options{E} =~ /amp/ ) {
        $t =~ s/&/&amp;/gs;
    }
    if ( !defined( $options{E} ) || $options{E} =~ /quot/ ) {
        $t =~ s/"/&quot;/gs;
    }
    if ( !defined( $options{E} ) || $options{E} =~ /apos/ ) {
        $t =~ s/'/&apos;/gs;
    }
...
sub printHeader {
    ...
    print $FH
"<tv source-info-url=\"http://tvlistings.zap2it.com/\" source-info-name=\"zap2it.com\"";
    print $FH
...
sub printProgrammes {
    ...
    for my $station ( sort sortChan keys %stations ) {
        my $i        = 0;
        my @keyArray = sort {
            $schedule{$station}{$a}{time} cmp $schedule{$station}{$b}{time}
        } keys %{ $schedule{$station} };
        foreach $s (@keyArray) {
            if ( $#keyArray <= $i
                && !defined( $schedule{$station}{$s}{endtime} ) )
            {
            ...
            print $FH
"\t<programme start=\"$startTime $startTZ\" stop=\"$stopTime $stopTZ\" channel=\""
              . &stationToChannel( $schedule{$station}{$s}{station} ) . "\">\n";
            ...
            if (
                defined( $programs{$p}{episode} )
                || (   defined( $options{M} )
                    && defined( $programs{$p}{movie_year} ) )
              )
            {
            ...
            print $FH "\t\t<desc lang=\"$lang\">"
              . &enc( $programs{$p}{description} )
              . "</desc>\n"
                ...
                || defined( $programs{$p}{preseter} ) )
            {
                ...
                foreach my $g (
                    sort {
                        $programs{$p}{actor}{$a} <=> $programs{$p}{actor}{$b}
                    } keys %{ $programs{$p}{actor} }
                  )
                {
                    print $FH "\t\t\t<actor";
                    print $FH " role=\""
                      . &enc( $programs{$p}{role}{$g} ) . "\""
                      if ( defined( $programs{$p}{role}{$g} ) );
            ...
            elsif ( defined( $programs{$p}{originalAirDate} )
                && $p =~ /^EP|^\d/ )
            {
            ...
            if ( defined( $programs{$p}{genres} ) ) {
                foreach my $g (
                    sort {
                        $programs{$p}{genres}{$a} <=> $programs{$p}{genres}{$b}
                          or $a cmp $b
                    } keys %{ $programs{$p}{genres} }
                  )
                {
                    print $FH "\t\t<category lang=\"$lang\">"
                      . &enc( ucfirst($g) )
                      . "</category>\n";
                }
            }

            print $FH "\t\t<length units=\"minutes\">"
              . $programs{$p}{duration}
              . "</length>\n"
            ...
            if (   defined( $programs{$p}{seasonNum} )
                && defined( $programs{$p}{episodeNum} ) )
            {
                ...
                if ( $s > 0 || $e > 0 ) {
                    print $FH "\t\t<episode-num system=\"common\">" . $sf . $ef
                      . "</episode-num>\n";
                }
            }
            ...
            if ( $dd_prog_id =~ /^(..\d{8})(\d{4})/ ) {
                ...
                print $FH "\t\t<episode-num system=\"dd_progid\">"
                  . $dd_prog_id
                  . "</episode-num>\n";
            }

            if ( defined($xs) && defined($xe) && $xs >= 0 && $xe >= 0 ) {
                print $FH "\t\t<episode-num system=\"xmltv_ns\">"
                  . $xs . "."
                  . $xe
                  . ".</episode-num>\n";
            }
            ...
            if ( defined( $schedule{$station}{$s}{premiere} ) ) {
                print $FH "\t\t<premiere>"
                  . $schedule{$station}{$s}{premiere}
                  . "</premiere>\n";
            }

            if ( defined( $schedule{$station}{$s}{finale} ) ) {
                print $FH "\t\t<last-chance>"
                  . $schedule{$station}{$s}{finale}
                  . "</last-chance>\n";
            }
...
sub printCredits {
    my ( $FH, $p, $s ) = @_;
    foreach my $g ( sort { $programs{$p}{$s}{$a} <=> $programs{$p}{$s}{$b} }
        keys %{ $programs{$p}{$s} } )
    {
        print $FH "\t\t\t<$s>" . &enc($g) . "</$s>\n";
    }
...
sub getURL {
    ...
    if ($er) {
        &perr(
            "Server out of data? Temporary server error? Normal exit anyway.\n"
        );
        return "";
    }
    die;
...
sub copyLogo {
    ...
    if ( defined($iconDir) && defined( $logos{$cid}{logo} ) ) {
        ...
        my $dest2 =
          "$iconDir/$num " . $stations{$key}{name} . $logos{$cid}{logoExt};
        my $dest3 =
          "$iconDir/$num\_" . $stations{$key}{name} . $logos{$cid}{logoExt};
        copy( $src, $dest1 );
        copy( $src, $dest2 );
        copy( $src, $dest3 );
    }
    ...
    foreach $s (@$sts) {
        ...
        if ( defined( $s->{'channelId'} ) ) {
            ...
            foreach $e (@$events) {
                ...
                $programs{$cp}{episode} = $program->{'episodeTitle'}
                  if ( $program->{'episodeTitle'} ne '' );
                $programs{$cp}{description} = $program->{'shortDesc'}
                  if ( $program->{'shortDesc'} ne '' );
                $programs{$cp}{duration} = $e->{duration}
                  if ( $e->{duration} > 0 );
                $programs{$cp}{movie_year} = $program->{releaseYear}
                  if ( $program->{releaseYear} ne '' );
                $programs{$cp}{seasonNum} = $program->{season}
                  if ( $program->{'season'} ne '' );
                ...
                $schedule{$cs}{$sch}{endTime} =
                  str2time1( $e->{'endTime'} ) * 1000;
                ...
                if ( $e->{'flag'} ) {
                    ...
                    if ( grep $_ eq 'New', @{$flags} ) {
                        $schedule{$cs}{$sch}{new} =
                          'New' & setOriginalAirDate();
                    }
                    if ( grep $_ eq 'Live', @{$flags} ) {
                        $schedule{$cs}{$sch}{live} =
                          'Live' & setOriginalAirDate();    # live to tape?
                    }
...
sub postJSONO {
    ...
    if ( !-e $fn ) {
        ...
        my $r =
          &ua_post( $url, \%phash, 'X-Requested-With' => 'XMLHttpRequest' );
        if ( $r->is_success ) {
            $dc =
              Encode::encode( 'utf8', $r->decoded_content( raise_error => 1 ) );
    ...
    if ( -e $fn ) {
        ...
        if ( $cp =~ /^MV|^SH/
            && length( $t->{seriesDescription} ) >
            length( $programs{$cp}{description} ) )
        {
