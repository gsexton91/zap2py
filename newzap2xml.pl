...
if (open (CONF, $confFile))
{
  ...
  while (<CONF>)
  {
    ...
    # --- Extended config flags mapped to CLI options ---
    elsif (/^\s*(\w+)\s*=\s*(.+)\s*$/i) {
      my ($key, $val) = (lc($1), &rtrim($2));
      my %map = (
        details   => 'D',   # include details
        icons     => 'I',   # include icons
        utf8      => 'U',   # UTF-8 output
        live      => 'L',   # output <live/>
        allchan   => 'a',   # all channels
        series    => 'j',   # add "series" category
        asterisk  => 'A',   # append "*" to new/live
        quiet     => 'q',   # quiet mode
        sleep     => 'S',   # sleep between requests
        tvguide   => 'z',   # use tvguide.com
      );

      if (exists $map{$key}) {
        my $flag = $map{$key};
        if ($val =~ /^(true|yes|on|1)$/i) {
          $options{$flag} = 1;
        } elsif ($val =~ /^(false|no|off|0)$/i) {
          delete $options{$flag};
        } else {
          $options{$flag} = $val; # numeric or text value
        }
      } else {
...
# --- auto-enable lineup mode and all channels when no login provided ---
if ($userEmail eq '' && !defined($zlineupId)) {
  # Use lineupId/postalcode from config if present
  $zlineupId = $lineupId if defined($lineupId);
  $zipcode   = $postalcode if defined($postalcode);

  # Automatically output all channels if -a not already set
...
my $s3 = time();
print "Completed in " . ( $s3 - $s1 ) . "s (Parse: " . ( $s2 - $s1 ) . "s) " . keys(%stations) . " stations, " . keys(%programs) . " programs, $ts scheduled.\n";
...
sub ua_stats {
  ...
  if (!defined $ua) {
    ...
    $ua->proxy(['http', 'https'], $proxy) if defined($proxy);
    $ua->agent($userAgent);
    $ua->default_headers->push_header('Accept-Encoding' => 'gzip, deflate');   # initialize LWP::UserAgent and cookies
  }
